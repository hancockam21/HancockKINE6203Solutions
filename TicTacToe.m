...
% Lines 317-328 are checking for a user win. If there is 3 in a row of X,
% it displays that the user won. 
    ...
    while posA == "X" && posB == "X" && posC == "X" || ... % the ... continues the code into the next line
          ...
          posC == "X" && posE == "X" && posH == "X" 

        disp("You Won!!!") 
        break % break statment ensures that You Won!!! isn't repeated over and over again. 
    end 
 

    % Lines 332-342 are checking for if the computer won. If there is 3 Os
    ...
    newGame = questdlg("Would you like to play again? Please enter Y or N as your response.")
    switch newGame
        case "N"
        continue 
        case "Y"
...
avail = setdiff(avail, unavail);   % updating available moves based on 
 ...
 avail = setdiff(avail, unavail) ;
    ...
    elseif avail(1, compMoveFour) == "I" \ "i"
        gameboard(3,3) = "O" ; 
        unavail = "I"; 
        disp(gameboard)
